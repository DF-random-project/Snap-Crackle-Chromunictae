...
import { sessionMiddleware, requireSession, requireAdmin } from './middleware/session';
import { syncAllUsers } from './lib/sync';
import { updateAnnouncement, buildAnnouncementBlocks } from '../lib/announcements';
...
export function createWebApp(env: Env) {
  ...
  api.get('/users', requireSession(), async (c) => {
    ...
    ).all<{ user_id: string; name: string; avatar_url: string; role: string | null; is_admin: number; cdt_id: string | null; cdt_name: string | null }>();
  ...
  api.post('/rsvp/:meetingId', requireSession(), async (c) => {
    ...
    if (!['yes', 'maybe', 'no'].includes(status)) return c.json({ error: 'Invalid status' }, 400);
    ...
    ).bind(meetingId, session.user_id, status, note).run();
  ...
  api.get('/admin/users/:userId/meetings', requireAdmin(), async (c) => {
    ...
    ).bind(userId).all<{ id: number; name: string; scheduled_at: number; status: string; note: string }>();
    return c.json(rows.results);
  });
  ...
  api.get('/admin/cdts', requireAdmin(), async (c) => {
    const rows = await c.env.DB.prepare(
      `SELECT c.id, c.name, c.handle, c.channel_id,
       ...
       GROUP BY c.id
       ORDER BY c.name COLLATE NOCASE`
    ).all<{ id: string; name: string; handle: string; channel_id: string; member_count: number }>();
    return c.json(rows.results);
  });
  ...
  api.get('/admin/cdts/:id', requireAdmin(), async (c) => {
    ...
    ).bind(id).first<{ id: string; name: string; handle: string; channel_id: string }>();
    ...
    const members = await c.env.DB.prepare(
      `SELECT u.user_id, u.name, u.avatar_url
       ...
       WHERE cm.cdt_id = ?
       ORDER BY u.name COLLATE NOCASE`
    ).bind(id).all<{ user_id: string; name: string; avatar_url: string }>();

    return c.json({ ...cdt, member_count: members.results.length, members: members.results });
  });

  api.post('/admin/cdts', requireAdmin(), async (c) => {
    const { name, handle, channel_id } = await c.req.json<{ name: string; handle?: string; channel_id?: string }>();
    if (!name) return c.json({ error: 'Name is required' }, 400);
    const finalHandle = handle || slugify(name);

    const adminClient = new SlackAPIClient(c.env.SLACK_ADMIN_TOKEN);
    const result = await adminClient.usergroups.create({ name, handle: finalHandle }) as { usergroup?: { id: string } };
    const groupId = result.usergroup?.id;
    if (!groupId) return c.json({ error: 'Failed to create Slack usergroup' }, 500);

    await c.env.DB.prepare(
      'INSERT INTO cdt (id, name, handle, channel_id) VALUES (?, ?, ?, ?)'
    ).bind(groupId, name, finalHandle, channel_id ?? null).run();

    const cdt = await c.env.DB.prepare(
      `SELECT c.id, c.name, c.handle, c.channel_id, 0 AS member_count FROM cdt c WHERE c.id = ?`
    ).bind(groupId).first<{ id: string; name: string; handle: string; channel_id: string; member_count: number }>();
  ...
  api.put('/admin/cdts/:id', requireAdmin(), async (c) => {
    ...
    if (body.channel_id !== undefined) { fields.push('channel_id = ?'); values.push(body.channel_id); }
    if (fields.length === 0) return c.json({ error: 'No fields to update' }, 400);
    values.push(id);
    await c.env.DB.prepare(`UPDATE cdt SET ${fields.join(', ')} WHERE id = ?`).bind(...values).run();
    return c.json({ ok: true });
  });
  ...
  api.get('/admin/meetings', requireAdmin(), async (c) => {
    ...
    ).all<{ id: number; name: string; description: string; scheduled_at: number; channel_id: string; message_ts: string; cancelled: number; series_id: number | null; yes_count: number; maybe_count: number; no_count: number }>();
    return c.json(rows.results.map(r => ({ ...r, cancelled: r.cancelled === 1 })));
  });

  api.post('/admin/meetings', requireAdmin(), async (c) => {
    const body = await c.req.json<{ name: string; description?: string; scheduled_at: number; channel_id?: string }>();
    ...
    const result = await c.env.DB.prepare(
      `INSERT INTO meeting (name, description, scheduled_at, channel_id, message_ts, cancelled)
       VALUES (?, ?, ?, ?, ?, 0)`
    ).bind(name, description, scheduled_at, channel_id ?? null, message_ts).run();

    const id = result.meta.last_row_id as number;
    return c.json({
      id, name, description, scheduled_at,
      channel_id: channel_id ?? null,
      message_ts,
      cancelled: false,
      series_id: null,
      yes_count: 0, maybe_count: 0, no_count: 0,
    }, 201);
  });

  api.put('/admin/meetings/:id', requireAdmin(), async (c) => {
    const id = Number(c.req.param('id'));
    const body = await c.req.json<{ name?: string; description?: string; scheduled_at?: number }>();
    const fields: string[] = [];
    ...
    if (fields.length === 0) return c.json({ error: 'No fields to update' }, 400);
    values.push(id);
    await c.env.DB.prepare(`UPDATE meeting SET ${fields.join(', ')} WHERE id = ?`).bind(...values).run();

    const meeting = await c.env.DB.prepare(
      'SELECT id, name, description, scheduled_at, channel_id, message_ts, cancelled FROM meeting WHERE id = ?'
    ).bind(id).first<{ id: number; name: string; description: string; scheduled_at: number; channel_id: string; message_ts: string; cancelled: number }>();
  ...
  api.post('/admin/meetings/series', requireAdmin(), async (c) => {
    ...
    const { name, description = '', scheduled_at, channel_id, days_of_week, time_of_day_minutes, end_date } = body;
    if (!name || !days_of_week.length || !end_date) return c.json({ error: 'Missing fields' }, 400);

    const { generateDates } = await import('../lib/recurrence');
    const dates = generateDates(days_of_week, time_of_day_minutes, scheduled_at, end_date);
    if (dates.length === 0) return c.json({ error: 'No occurrences generated' }, 400);
    ...
    for (const ts of dates) {
      ...
      const result = await c.env.DB.prepare(
        `INSERT INTO meeting (series_id, name, description, scheduled_at, channel_id, message_ts, cancelled)
         VALUES (?, ?, ?, ?, ?, ?, 0)`
      ).bind(seriesId, name, description, ts, channel_id ?? null, message_ts).run();
      created.push({ id: result.meta.last_row_id as number, scheduled_at: ts, message_ts });
    }
  ...
  api.post('/admin/users/bulk/role', requireAdmin(), async (c) => {
    ...
    const validRoles = ['student', 'mentor', 'parent', 'alumni'];
    if (role && !validRoles.includes(role)) return c.json({ error: 'Invalid role' }, 400);
    const stmt = c.env.DB.prepare('UPDATE slack_user SET role = ? WHERE user_id = ?').bind(role ?? null, null!);
    await c.env.DB.batch(
      user_ids.map(id => c.env.DB.prepare('UPDATE slack_user SET role = ? WHERE user_id = ?').bind(role ?? null, id))
    );
    return c.json({ ok: true });
  });

  api.post('/admin/users/bulk/cdt', requireAdmin(), async (c) => {
    const { user_ids, cdt_id } = await c.req.json<{ user_ids: string[]; cdt_id: string | null }>();
    if (!user_ids?.length) return c.json({ error: 'No user IDs provided' }, 400);
    if (cdt_id === null) {
      await c.env.DB.batch(
        user_ids.map(id => c.env.DB.prepare('DELETE FROM cdt_member WHERE user_id = ?').bind(id))
      );
    } else {
      await c.env.DB.batch(
        user_ids.map(id => c.env.DB.prepare(
          `INSERT INTO cdt_member (user_id, cdt_id) VALUES (?, ?)
           ON CONFLICT (user_id) DO UPDATE SET cdt_id = excluded.cdt_id`
        ).bind(id, cdt_id))
      );
