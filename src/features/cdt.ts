...
function buildListModal(
  ...
  return {
    ...
    ...(adminUser ? { submit: { type: 'plain_text', text: 'Create CDT' } } : {}),
    blocks,
  };
}
    ...
    blocks: [
      {
        type: 'input',
        block_id: 'name_block',
        element: { type: 'plain_text_input', action_id: 'cdt_name', initial_value: cdtRow.name },
        label: { type: 'plain_text', text: 'CDT Name' },
      },
      {
        type: 'input',
        block_id: 'channel_block',
        element: { type: 'channels_select', action_id: 'cdt_channel', initial_channel: cdtRow.channel_id },
        label: { type: 'plain_text', text: 'Channel' },
      },
      {
        type: 'input',
        ...
        elements: [
          {
            ...
            confirm: {
              ...
              text: { type: 'mrkdwn', text: `This will permanently remove *${cdtRow.name}* from all member profiles and free up the name in Slack.` },
...
const cdt = async (slackApp: SlackApp<SlackEdgeAppEnv>, env: Env) => {
  ...
  slackApp.action('cdt_open_edit', async ({ context, payload }) => {
    ...
    const members = await env.DB.prepare('SELECT user_id FROM cdt_member WHERE cdt_id = ?')
      .bind(cdtId).all<{ user_id: string }>();

    await context.client.views.push({
      trigger_id: (payload as any).trigger_id,
      view: buildEditModal(cdtRow, members.results.map(r => r.user_id)),
    });
  });
      ...
      context.client.views.update({
        ...
        view: {
          ...
          blocks: [
            {
              type: 'section',
              text: { type: 'mrkdwn', text: `*${cdtRow.name}* has been deleted and removed from all member profiles.` },
            },
          ],
        },
      }),
      rootViewId ? context.client.views.update({
        view_id: rootViewId,
  ...
  slackApp.viewSubmission(
    ...
    async (req) => {
      ...
      const [existingName, ugList] = await Promise.all([
        ...
        req.context.client.usergroups.list({ include_disabled: true, include_users: true }),
      ]);

      if (existingName) {
        return { response_action: 'errors', errors: { name_block: 'A CDT with this name already exists.' } };
      }

      const usergroups: any[] = (ugList as any).usergroups ?? [];
      const existingUg = usergroups.find((ug) => ug.handle === handle || ug.name === name);

      if (existingUg) {
        return { response_action: 'errors', errors: { name_block: 'A Slack usergroup with this name or handle already exists.' } };
      }
    ...
    async (req) => {
      try {
        ...
        const ugRes = await adminClient.usergroups.create({ name, handle, channels: channelId });
        const usergroupId = (ugRes as any).usergroup.id;

        await env.DB.prepare('INSERT INTO cdt (id, name, handle, channel_id) VALUES (?, ?, ?, ?)')
          .bind(usergroupId, name, handle, channelId).run();

        for (const userId of members) {
          await env.DB.prepare(
            'INSERT INTO cdt_member (user_id, cdt_id) VALUES (?, ?) ON CONFLICT (user_id) DO UPDATE SET cdt_id = excluded.cdt_id'
          ).bind(userId, usergroupId).run();
          await setProfile(adminClient, userId, { [CDT_FIELD_ID]: name });
        }

        if (members.length > 0) {
          await adminClient.usergroups.users.update({ usergroup: usergroupId, users: members });
        }
  ...
  slackApp.viewSubmission(
    ...
    async (req) => {
      try {
        ...
        const currentRows = await env.DB.prepare('SELECT user_id FROM cdt_member WHERE cdt_id = ?')
          .bind(cdtId).all<{ user_id: string }>();
        ...
        await env.DB.prepare('UPDATE cdt SET name = ?, channel_id = ? WHERE id = ?')
          .bind(newName, newChannelId, cdtId).run();
        ...
        for (const userId of added) {
          await env.DB.prepare(
            'INSERT INTO cdt_member (user_id, cdt_id) VALUES (?, ?) ON CONFLICT (user_id) DO UPDATE SET cdt_id = excluded.cdt_id'
          ).bind(userId, cdtId).run();
          await setProfile(adminClient, userId, { [CDT_FIELD_ID]: newName });
        }

        for (const userId of removed) {
          await env.DB.prepare('DELETE FROM cdt_member WHERE user_id = ? AND cdt_id = ?')
            .bind(userId, cdtId).run();
