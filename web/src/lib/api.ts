...
export const api = {
  ...
  async rsvp(meetingId: number, status: 'yes' | 'maybe' | 'no', note = ''): Promise<void> {
  ...
  async createCdt(data: { name: string; handle?: string; channel_id?: string }): Promise<Cdt> {
    return (await apiFetch('/api/admin/cdts', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data),
    })).json()
  },
  async updateCdt(id: string, data: { name?: string; channel_id?: string }): Promise<void> {
  ...
  async createMeeting(data: { name: string; description?: string; scheduled_at: number; channel_id?: string }): Promise<AdminMeeting> {
    return (await apiFetch('/api/admin/meetings', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data),
    })).json()
  ...
  async createMeetingSeries(data: {
    name: string; description?: string; scheduled_at: number; channel_id?: string;
    days_of_week: number[]; time_of_day_minutes: number; end_date: number;
  }): Promise<AdminMeeting> {
    return (await apiFetch('/api/admin/meetings/series', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data),
    })).json()
  },
  async updateMeeting(id: number, data: { name?: string; description?: string; scheduled_at?: number }): Promise<void> {
